...
describe('Receipt Proof Verification (Hybrid: @zkpersona/noir-helpers + bb CLI)', () => {
  let prover: Prover;
  let publicClient: PublicClient;

  const bbPath = '/root/.bb/bb';
  const circuitPath = path.join(process.cwd(), 'target/verify_receipt.json');
  const outputDir = path.join(process.cwd(), 'target/verify_receipt');
  ...
  it('should prove receipt proof using hybrid approach', async () => {
    ...
    console.log('✅ Receipt proof verified successfully (Hybrid approach)');
  });
});
