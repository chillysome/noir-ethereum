...
describe('Storage Proof Verification (Hybrid: @zkpersona/noir-helpers + bb CLI)', () => {
  let prover: Prover;
  let publicClient: PublicClient;

  const bbPath = '/root/.bb/bb';
  const circuitPath = path.join(process.cwd(), 'target/verify_storage.json');
  const outputDir = path.join(process.cwd(), 'target/verify_storage');
  ...
  it('should prove contract storage proof using hybrid approach', async () => {
    ...
    console.log(
      '✅ Contract storage proof verified successfully (Hybrid approach)'
    );
  });
});
