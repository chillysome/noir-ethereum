...
describe('Transaction Proof Verification (Hybrid: @zkpersona/noir-helpers + bb CLI)', () => {
  ...
  const circuitPath = path.join(
    process.cwd(),
    'target/verify_transaction.json'
  );
  const outputDir = path.join(process.cwd(), 'target/verify_transaction');
  ...
  it('should prove transaction proof using hybrid approach', async () => {
    ...
    console.log('✅ Transaction proof verified successfully (Hybrid approach)');
  });
});
