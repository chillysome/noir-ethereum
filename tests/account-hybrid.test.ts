...
describe('Account Proof Verification (Hybrid: @zkpersona/noir-helpers + bb CLI)', () => {
  let prover: Prover;
  let publicClient: PublicClient;

  const bbPath = '/root/.bb/bb';
  const circuitPath = path.join(process.cwd(), 'target/verify_account.json');
  const outputDir = path.join(process.cwd(), 'target/verify_account');
  ...
  it('should prove account proof using hybrid approach', async () => {
    ...
    console.log('✅ Account proof verified successfully (Hybrid approach)');
  });
});
